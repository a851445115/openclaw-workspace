...
export async function handleFeishuMessage(params: {
  ...
  const sendFallbackFinal = async (text: string, reason: string) => {
    try {
      ...
      log(`feishu[${account.accountId}]: fallback final sent (${reason})`);
    } catch (sendErr) {
      error(
        `feishu[${account.accountId}]: failed to send fallback final (${reason}): ${String(sendErr)}`,
      );
    }
  };
  ...
  try {
    ...
    await sendFallbackFinal(DISPATCH_FAILED_FALLBACK_TEXT, "dispatch-error");
  }
}
